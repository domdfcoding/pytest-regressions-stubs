...
def test_usage_workflow(testdir, monkeypatch):
	...
	monkeypatch.setattr(sys, "testing_get_data", lambda: {"data": 1.1 * np.ones(50)}, raising=False)
	source = """
        import sys
	...
	check_regression_fixture_workflow(
			...
			setattr(sys, "testing_get_data", lambda: {"data": 1.2 * np.ones(50)}, raising=False),
...
def test_arrays_with_different_sizes(num_regression, no_regen):
	...
	with pytest.raises(AssertionError, match="Obtained and expected data shape are not the same."):
		num_regression.check({"data1": data1})
