...
class ImageRegressionFixture:
	...
	def _check_images_manhattan_distance(self, obtained_file, expected_file, expect_equal, diff_threshold):
		"""
		...
		def check_result(equal, manhattan_distance):
			if equal != expect_equal:
				...
				if expect_equal:
					assert 0, ("Difference between images too high: %.2f %%\n%s\n%s" % params)
				else:
					assert 0, ("Difference between images too small: %.2f %%\n%s\n%s" % params)
