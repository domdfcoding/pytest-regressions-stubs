# 3rd party
from pytest_regressions.common import import_error_message, perform_regression_check
...
class NumericRegressionFixture:
	...
	def _check_data_types(self, key, obtained_column, expected_column):
		...
		if obtained_data_type != expected_data_type:
			...
			if np.issubdtype(obtained_data_type, np.number) and np.issubdtype(expected_data_type, np.number):
				return
	...
	def _check_fn(self, obtained_filename, expected_filename):
		...
		for k in obtained_data.keys():
			...
			if np.any(not_close_mask):
				...
				comparison_table = pd.concat([diff_obtained_data, diff_expected_data, diffs], axis=1)
	...
	def check(
		...
		for obj in data_dict.values():
			assert type(obj) in [np.ndarray], "Only numpy arrays are valid for numeric_data_regression fixture.\n"
			shape = obj.shape
		...
		if not np.all(data_shapes == data_shapes[0]):
			...
			elif len(data_dict) > 1 and not all(np.issubdtype(a.dtype, np.floating) for a in data_dict.values()):
				raise TypeError(
